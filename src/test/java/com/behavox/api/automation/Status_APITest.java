...
public class Status_APITest extends BaseTest{
    ...
    public void StatusAPITest_PENDING() throws InterruptedException {
        ...
        SubmitEPResponse submitSubmitCallResponse = getGson().fromJson(secondCallResponse.getBody().asString(), SubmitEPResponse.class);

        Response getStatusResponse = executeGetStatusReq(httpRequestTwo, submitSubmitCallResponse.getId());
        StatusEPResponse statusEPResponse = new Gson().fromJson(getStatusResponse.getBody().asString(), StatusEPResponse.class);


        Assert.assertEquals(secondCallResponse.getStatusCode(), 200);
        Assert.assertNotNull(submitSubmitCallResponse);
        Assert.assertEquals(statusEPResponse.getStatus(),"PENDING","Third request is queued, Request queue functionality is  passed!!");
      	Thread.sleep(7000);
    }
    ...
    public void StatusAPITest_FAILED() throws InterruptedException {
        ...
        SubmitEPResponse submitSubmitCallResponse = getGson().fromJson(callResponse.getBody().asString(), SubmitEPResponse.class);
        Thread.sleep(3000);
        ...
        Assert.assertNotNull(submitSubmitCallResponse);
        Assert.assertEquals(statusEPResponse.getStatus(),"FAILED","Request is successful with status as failed, Request queue functionality is  passed!!");
    }
