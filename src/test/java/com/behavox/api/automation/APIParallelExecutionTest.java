...
public class APIParallelExecutionTest extends BaseTest{
    ...
    public void ExecuteTwoRequestsTest_WhileFirstOneInProgress() throws InterruptedException {
      ...
      //Execute second submit request
        ...
        SubmitEPResponse secondSubmitCallResponse = new Gson().fromJson(secondCallResponse.getBody().asString(), SubmitEPResponse.class);

        Response getStatusResponse = executeGetStatusReq(httpRequestTwo, secondSubmitCallResponse.getId());
        StatusEPResponse secondCallStatusResponse = getGson().fromJson(getStatusResponse.getBody().asString(), StatusEPResponse.class);


        Assert.assertEquals(secondCallResponse.getStatusCode(), 200);
        Assert.assertNotNull(secondSubmitCallResponse);
        Assert.assertNotEquals(secondCallStatusResponse.getStatus(),"PENDING","Second request is processed while first one is still IN-PROGRESS, Parallel execution for 2 requests passed!!");
        Thread.sleep(8000);
    }
    ...
    public void ExecuteThreeRequestsTest_WhileFirstTwoInProgress() throws InterruptedException {
        ...
        SubmitEPResponse thirdSubmitCallResponse = getGson().fromJson(thirdCallResponse.getBody().asString(), SubmitEPResponse.class);

        Response getStatusResponse = executeGetStatusReq(httpRequestTwo, thirdSubmitCallResponse.getId());
        StatusEPResponse thirdSubmitStatusResponse = new Gson().fromJson(getStatusResponse.getBody().asString(), StatusEPResponse.class);


        Assert.assertEquals(thirdCallResponse.getStatusCode(), 200);
        Assert.assertNotNull(thirdSubmitCallResponse);
        Assert.assertEquals(thirdSubmitStatusResponse.getStatus(),"PENDING","Third request is queued, Request queue functionality is  passed!!");
        Thread.sleep(10000);
    }
}
